...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Batch-generate images using PhotoMaker pipeline based on prompts in a CSV file."
    )
    parser.add_argument("--csv_file", type=str, default="/disk1/fjm/UniPortrait/assets/faceprompt.csv",
                        help="Path to CSV file with two columns: image_name,prompt")
    parser.add_argument("--image_folder", type=str, default="/disk1/fjm/img",
                        help="Folder containing input images referenced in CSV")
    parser.add_argument("--output_folder", type=str, default="/disk1/fjm/resultimg",
                        help="Directory to save generated images")
    parser.add_argument("--start_row", type=int, default=0,
                        help="1-based index of the first CSV row to process (inclusive)")
    parser.add_argument("--end_row", type=int, default=5,
                        help="1-based index of the last CSV row to process (inclusive). Default=last row")
...
def main():
    ...
    for idx, (image_name, prompt_text) in df.iterrows():
        # Seed
        ...
        for i, out in enumerate(outputs, start=1):
            save_path = os.path.join(args.output_folder, f"{i}_{base}_{styled_p}.png")
            out.save(save_path)
